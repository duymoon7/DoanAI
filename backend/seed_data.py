...
def seed_database():
    ...
    try:
        ...
        existing_users = db.query(NguoiDung).count()
        if existing_users > 0:
            logger.warning(f"⚠️  Database already has {existing_users} users")
            response = input("Do you want to clear and reseed? (yes/y/no): ")
